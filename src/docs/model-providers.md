...
百炼是阿里云的大模型服务平台，提供通义千问等模型，使用 DashScope 的 OpenAI 兼容接口。需在 `models.providers` 中显式配置 `baseUrl`、`apiKey` 和 `models`。
...
```json
{
  ...
  "agents": {
    "defaults": { "model": { "primary": "bailian/qwen3.5-flash" } }
  }
}
```

- **API Key**：在 [阿里云百炼控制台](https://bailian.console.aliyun.com/) 创建应用并获取 API Key，可配置为 `$DASHSCOPE_API_KEY` 环境变量或直接填写字面量。
- **Base URL**：`https://dashscope.aliyuncs.com/compatible-mode/v1` 为 DashScope OpenAI 兼容模式端点。
- **常用模型**：`qwen3.5-flash`、`qwen3.5-plus` 等，具体以百炼平台文档为准。
