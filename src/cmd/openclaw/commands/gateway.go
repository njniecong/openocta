...
import (
	...
	"github.com/openclaw/openclaw/pkg/daemon"
	gatewayclient "github.com/openclaw/openclaw/pkg/gateway/client"
	gatewayhttp "github.com/openclaw/openclaw/pkg/gateway/http"
	"github.com/openclaw/openclaw/pkg/infra"
...
func runGateway(cmd *cobra.Command, _ []string) error {
	...
	// Init global logger: console + /tmp/openclaw/openclaw-YYYY-MM-DD.log (JSON, daily rolling)
...
func runGatewayInstall(cmd *cobra.Command, _ []string) error {
	...
	if err := daemon.Bootstrap(label); err != nil {
		cmd.Printf("Plist written; start with: openclaw gateway restart\n")
		return nil
	}
...
func runGatewayRestart(cmd *cobra.Command, _ []string) error {
	...
	if _, err := os.Stat(plistPath); os.IsNotExist(err) {
		return fmt.Errorf("gateway not installed; run openclaw gateway install first")
	}
	loaded, _ := daemon.IsLoaded(label)
	if !loaded {
		if err := daemon.Bootstrap(label); err != nil {
